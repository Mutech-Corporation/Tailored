...
export function ServiceCard({ variant, icon: Icon, phoneLabel, title, messages, description }: ServiceCardData) {
  return (
    <div className="tw-lift flex h-full flex-col gap-3 rounded-2xl border border-[#dfe3e8] bg-white p-5 px-[1.25rem] shadow-[0_4px_12px_rgba(0,0,0,0.04)] hover:border-[#2563eb]">
      <div
        ...
        <div className={`flex min-h-[240px] w-full flex-1 flex-col overflow-hidden rounded-[24px] p-4 shadow-[0_8px_24px_rgba(0,0,0,0.15)] sm:min-h-[260px] lg:min-h-[280px] ${GRADIENT[variant]}`}>
          <div className="flex min-h-[210px] flex-1 flex-col rounded-2xl bg-white p-4 sm:min-h-[230px] lg:min-h-[250px]">
            <div className="mb-4 flex items-center gap-2 border-b border-[#e5e7eb] pb-3">
              <Bot className="size-[1.2rem] text-[#2563eb]" />
      ...
      <Link
        href="/contact"
        className="mt-2 flex w-full items-center justify-center rounded-full bg-[#2563eb] px-[1.6rem] py-[0.65rem] text-[0.95rem] font-semibold text-white transition-[transform,background-color] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-0.5"
      >
        Request Access
